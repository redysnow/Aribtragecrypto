...
max_price = float(prices[platform_max])
min_price = float(prices[platform_min])

# Calcul de l'écart
ecart = max_price - min_price

# Calcul de l'écart en pourcentage
diff_percentage = (ecart / min_price) * 100

print(f"Le plus grand écart est entre {platform_max} ({max_price}) et {platform_min} ({min_price}) avec une différence de {ecart} ({diff_percentage:.2f}%).")
